...
**NOTE** For MPI users: Currently, this executable runs on only one slot (`mpiexec -n 1`). If more than one slot is provided, processes with MPI ranks greater than 0 will terminate.
...
- `--old_behavior`: Simulate the behavior of the original ART profile builder. If set, all qualities will be offset by 1. See also: [The Bug in `Srcurity.md`](#original-art-profile-builder-bug).
...
- The input SAM/BAM files do **NOT** need to be sorted.
...
- Automatic format detection is supported. However:
  ...
  - The program will first try to detect the SRA format if supported. If not, the program will try to tell the format using HTSLib. See also: [`htsfile(1)`](https://www.htslib.org/doc/htsfile.html).
